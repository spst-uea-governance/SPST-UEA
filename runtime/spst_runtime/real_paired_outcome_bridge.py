...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect or review a pre-registered real paired outcome program."
    )
    ...
    for name in ("get", "status", "review"):
        ...
        if name == "review":
            ...
            command.add_argument("--blind-review-surface-sha256", required=True)
...
def _review_payload(arguments: argparse.Namespace) -> dict[str, Any]:
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments.command == "get":
        output = program.get(arguments.program_id) or {
            "schema": "spst-real-paired-outcome-program-v1",
            "status": "not_found",
            "id": arguments.program_id,
        }
    elif arguments.command == "status":
        output = {"coverage": program.coverage(), "history": program.history()}
    else:
        output = program.review(
            arguments.program_id,
            _review_payload(arguments),
        )
