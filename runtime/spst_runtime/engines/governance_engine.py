...
class GovernanceEngine:
    ...
    def decide(self, action: Any) -> dict[str, Any]:
        ...
        if not isinstance(action, dict):
            return self._decision("Low", False, False, ["invalid_action"], action={})

        security = action.get("security_assessment", {})
        if security.get("blocked"):
            return self._decision("Low", False, False, ["security_red_team_block"], action=action)
        ...
        if action_type == "commit_identity_change" and source == "model_output_direct":
            return self._decision("Low", False, False, ["direct_model_identity_mutation"], action=action)
        if action_type in {"commit_identity_change", "state_transition"} and not provenance_verified:
            return self._decision("Low", False, False, ["unverified_provenance"], action=action)
        if breaking:
            ...
            return self._decision(
                ...
                approved,
                not approved,
                ["breaking_or_architectural_change"],
                action=action,
            )
        return self._decision("High", True, False, [], action=action)
